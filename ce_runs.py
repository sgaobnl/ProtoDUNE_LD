...
class CE_RUNS:
    ...
    def WIB_PLL_wr(self, wib_ip, addr, din):
        ...
        #value = 0x01 + ((addr&0xFF)<<8) + ((din&0x00FF)<<16)
        self.femb_meas.femb_config.femb.write_reg_wib (12,addr)
        self.femb_meas.femb_config.femb.write_reg_wib (13,din)
        self.femb_meas.femb_config.femb.write_reg_wib (11,1)
        time.sleep(0.01)
        self.femb_meas.femb_config.femb.write_reg_wib (10,4)
        time.sleep(0.01)
    ...
    def WIB_PLL_cfg(self ):
        ...
        for wib_ip in self.wib_ips:
            ...
            if(True):
                ...
                self.femb_meas.femb_config.femb.write_reg_wib (10,0x0FF0)
                time.sleep(0.01)
                self.femb_meas.femb_config.femb.write_reg_wib (10,0x0FF0)
                time.sleep(0.1)
                p_addr = 1
